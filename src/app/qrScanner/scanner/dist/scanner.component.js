"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
exports.__esModule = true;
exports.ScannerComponent = void 0;
var core_1 = require("@angular/core");
var ScannerComponent = /** @class */ (function () {
    function ScannerComponent(qrdataService) {
        var _this = this;
        this.qrdataService = qrdataService;
        this.qrCodeImage = '';
        // Subscribe to changes in the QR code image data
        this.qrCodeImageSubscription = this.qrdataService.qrCodeImage$.subscribe(function (image) {
            _this.qrCodeImage = image;
        });
    }
    ScannerComponent.prototype.ngOnDestroy = function () {
        // Unsubscribe to avoid memory leaks
        this.qrCodeImageSubscription.unsubscribe();
    };
    ScannerComponent = __decorate([
        core_1.Component({
            selector: 'app-scanner',
            templateUrl: './scanner.component.html',
            styleUrls: ['./scanner.component.scss']
        })
    ], ScannerComponent);
    return ScannerComponent;
}());
exports.ScannerComponent = ScannerComponent;
